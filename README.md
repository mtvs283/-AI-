...
- `onmaeum-inquiry-answer-fix.sql`은 관리자 답변이 저장되지 않던 문제를 고칩니다. `onmaeum_answer_inquiry`가 존재하지 않는 `onmaeum_inquiry_settings.resend_api_key`를 읽어 함수 전체가 실패했습니다. 메일 키는 Vault의 `onmaeum_resend_api_key`를 쓰고, 발송이 실패해도 답변은 남깁니다.
